...
# for emission lines; not strictly ions 
# (Serena Bertone and Freeke van de Voort line choices)   
# O VII triplet names: 
# https://www.aanda.org/articles/aa/pdf/2015/07/aa26324-15.pdf 
# and "Metal-line emission from the WHIM: 
# I. Soft X-rays" (Bertone et al. 2010) 
# intercombination x line from Bertone et al. energy 
# and Kaastra et al. wavelength ratio         
# o6minor same 
line_eng_ion = {'c5r': 307.88*c.ev_to_erg,
                'c6': 367.47*c.ev_to_erg,
                'n6f': 419.86*c.ev_to_erg,
                'n6r': c.planck * (c.c / (28.79 * 1e-8)),
                'n7': 500.24*c.ev_to_erg,
                'o7r': 573.95*c.ev_to_erg,
                'o7i': 568.74*c.ev_to_erg,
...
# save images to:
mdir = './imgs/'
# base directory for response and background files
dir_insfiles = './instrumentfiles/'

# response files and backgrounds:
ddir_xifu = dir_insfiles + 'athena_x-ifu/cost-constrained_2020-09-28/'
...
full_lynx_mucal_hires_rmf = dir_insfiles + 'lynx/mucal/xrs_mucal_0.3eV.rmf'
full_lynx_mucal_hires_arf = dir_insfiles + \
                            '/soxs_responses/xrs_mucal_3x10_0.3eV.arf'
full_lynx_mucal_main_rmf = dir_insfiles + 'lynx/mucal/xrs_mucal_3.0eV.rmf'
full_lynx_mucal_main_arf = dir_insfiles + \
                           '/soxs_responses/xrs_mucal_3x10_3.0eV.arf'

# multiple background files
basedir_bkg_lynx = dir_insfiles + 'lynx/mucal/'
...
ddir_xrism_resolve = dir_insfiles + 'xrism/resolve/specfiles_v002/'
# arf for a constant SB over pi*5*5 arcmin area (circle), 
# not 3x3 arcmin2 FOV
filename_arf_xrism_resolve = 'resolve_flt_spec_noGV_20190611.arf' 
filename_bkg_nonxray_xrism_resolve = 'resolve_h5ev_2019a_rslnxb.pha'
filename_rmf_xrism_resolve = 'resolve_h5ev_2019a.rmf'
filename_bkg_xray_xrism_resolve = dir_insfiles +\
    'bkgmodel_astro_aurora_2020-10-20_resolve_h5ev_2019a.fak'
...
class Responses:
    ...
    def check_compat(self):
        ...
        if not (np.allclose(self.rmf.energ_lo, self.E_lo_arf) and\
            ...
            msg = 'Energy channels in the .arf and .rmf files do not match'
        ...
        if self.instrument == 'athena-xifu':
            ...
            for fn in [self.bkg_fn1, self.bkg_fn2, self.bkg_fn3]:
                with fits.open(fn) as hdu:
                    ...
                    minl = self.responses.channel_rmf[-1] + 1
                    bc = np.bincount(channels, minlength=minl)
            ...
            if bkg_fn is None:
                self.bkg_fn1 = ddir_xrism_resolve + \
                               filename_bkg_nonxray_xrism_resolve
                ...
                msg = ('Non-standard background files are not an option for'
                       ' XRISM Resolve given extraction area issues')
                raise ValueError(msg)
            
            _bkg = np.zeros(self.responses.rmf.detchans, dtype=np.float)
            with fits.open(self.bkg_fn1) as hdu:
                self.channel_bkg = hdu[1].data['CHANNEL'] 
                # counts / channel, no rate data
                self.rate_bkg = hdu[1].data['COUNTS'] 
...
def getminSB_grid(E_rest, linewidth_kmps=100., z=0.0,
    ...
    if instrument == 'athena-xifu':
        ...
        msg = '{} is not a valid instrument option'
    ...
    if isinstance(extr_range, int):
        ...
        extr_range *= 1e-3 # eV to keV
        mins = np.argmin(np.abs(E_lo[np.newaxis, :] \
                                - E_pos[:, np.newaxis] + extr_range), 
                         axis=1)
        maxs = np.argmin(np.abs(E_hi[np.newaxis, :] \
                                - E_pos[:, np.newaxis] - extr_range), 
                         axis=1)
...
def explorepars_omegat_extr(instrument):
    ...
    omegat = [1e5, 1e6, 1e7]
    title = ('Varying $\\Delta \\Omega \\times \\mathrm{t}_{\\mathrm{exp}}$'
             ' and the $\\Delta$E range for line measurment')
    fontsize = 12
    ...
    ax.set_ylabel('log10 min. SB [photons / s / cm**2 / sr]', 
                  fontsize=fontsize)
    ax.tick_params(which='both', direction='in', top=True, right=True,
                   left=True, bottom=True)
    ax.grid(b=True)
    ax.legend(fontsize=fontsize - 2)
    fn = 'minSB_{instr}_varying_omegatexp_spec-extr.pdf'
    plt.savefig(mdir + fn.format(instr=instrument), bbox_inches='tight')        
...
def plot_minSB():
    ...
    ylabel = ('$\\min \\mathrm{SB} \\; [\\mathrm{ph} \\;'
              ' \\mathrm{s}^{-1} \\mathrm{cm}^{-2} \\mathrm{sr}^{-1}]$')
    ax.set_ylabel(ylabel, fontsize=fontsize)
    ax.set_xticks([0.2, 0.3, 0.5, 1., 2., 3.])
    ax.xaxis.set_major_formatter(ScalarFormatter())
    
    dest = ', $\\Delta$E = {de:.1f} eV'
    handles1 = [mlines.Line2D([], [], color=colors[isn],
                              label=labels[isn] +\
                                    dest.format(de=extr_ranges[isn][0]),
                             )\
                for isn in names ]
    otst = '{:.0e} $\\mathrm{{arcmin}}^{{2}}$ s'
    handles2 = [mlines.Line2D([], [],
                         label=otst.format(omt),
...
def save_minSB_grid():
    ...
    names = ['athena-xifu', 'lynx-lxm-main', 
             'lynx-lxm-uhr', 'xrism-resolve']
    extr_ranges = {'athena-xifu': [2.5],
                   'lynx-lxm-main': [4.0],
                   'lynx-lxm-uhr': [0.6],
                   'xrism-resolve': [10.],
    ...
    fmtstring = ('{E}\t{minSB}\t{galabs}\t{areatime}'
                 '\t{linew}\t{nsigma}\t{extr}\n')
    info_wabs = ('#galactic absorption (if True) comes from\n'
                 '#the  McCammon et al. (2002) diffuse X-ray background'
                  ' model,\n'
                 '#using wabs (xspec model) for the galactic  absorption,\n'
                 '#with a hydrogen column of 1.8e20 cm**-2'
                 '(0.018 parameter value)\n')
    colnames = {'E': 'input line energy [keV]',
                'minSB': ('minimum detectable SB'
                ...
                'extr': ('signal/noise extraction region in the spectrum'
                         '[full width, eV]'),
                }
    columns = fmtstring.format(**colnames)
...
def plot_Aeff_galabs():
    names = ['athena-xifu', 'lynx-lxm-main', 'xrism-resolve'] 
    labels = {'athena-xifu': 'X-IFU',
              'lynx-lxm-main': 'LXM',
              'lynx-lxm-uhr': 'LXM-UHR',
              'xrism-resolve': 'XRISM-R',
    ...
    plt.savefig(mdir + 'Aeff_galabs_instruments_varying_omegatexp.pdf', 
                bbox_inches='tight')
    ...
    for isn in names:
        ...
        if isn == 'athena-xifu':
            ...
            with open(outname_wabs, 'w') as fo:
                fo.write(info_wabs)
                fo.write(fmtstring.format(E='energy [keV]', 
                                          Aeff='transmitted fraction'))
...
def plot_equiv_backgrounds(aeff_norm=True):
    '''
    backgrounds are smoothed for legibility, 
    but at higher energies (> ~ 7 keV) 
    the smoothing for the X-IFU will have a variable length 
    (non equally spaced energy grid)
    '''
    names = ['athena-xifu', 'lynx-lxm-main', 'lynx-lxm-uhr', 'xrism-resolve']
    labels = {'athena-xifu': 'X-IFU',
              'lynx-lxm-main': 'LXM-main',
    ...
    for isn in names:
        ins = InstrumentModel(instrument=isn)
        Egrid = 0.5 * (ins.responses.rmf.e_min + ins.responses.rmf.e_max) 
        ...
        # equally spaced at low energies for these instruments; 
        # smoothing will have an irregular spacing at high energies
        # 0 may be off if lowest bin energy had to be adjusted
        deltaE = deltaE_full[1] 
    ...
    ax.set_xlabel('E [keV]', fontsize=fontsize)
    if aeff_norm:
        ylabel = ('background est. $[\\mathrm{cts} \\, '
                  '\\mathrm{s}^{-1}\\mathrm{cm}^{-2}\\mathrm{arcmin}^{-2} '
                  '\\mathrm{keV}^{-1}]$')
        ax.set_ylabel(ylabel, fontsize=fontsize) 
        ...
        ylabel = ('background est. $[\\mathrm{cts} \\, \\mathrm{s}^{-1}'
                  '\\mathrm{arcmin}^{-2} \\mathrm{keV}^{-1}]$')
        ax.set_ylabel(ylabel, fontsize=fontsize) 
...
def checkvals_lynx_lxm_uhr():
    ...
    ax.set_xlabel('E [keV]', fontsize=fontsize)
    ylabel = ('min. SB $[\\mathrm{photons} \\, \\mathrm{s}^{-1}'
              '\\mathrm{cm}^{-2} \\mathrm{sr}^{-2}]$')
    ax.set_ylabel(ylabel, fontsize=fontsize)
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.legend(fontsize=fontsize - 2)
